...
function NewStudentDialog({ onClose, onCreated }: { onClose: () => void; onCreated: (r: NewStudentResult) => void }) {
  ...
  const create = useMutation({
    ...
    onSuccess: (result: Partial<NewStudentResult>) => {
      qc.invalidateQueries({ queryKey: ['pt-students'] })
      onClose()
      onCreated({ name: form.name, email: form.email, phone: form.phone, tempPassword: result.tempPassword, emailSent: result.emailSent })
    },
...
export default function PTStudentsPage() {
  ...
  const [newResult, setNewResult] = useState<NewStudentResult | null>(null)
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-5 max-w-5xl mx-auto">
      ...
      {/* New student dialog */}
      {showNew && <NewStudentDialog onClose={() => setShowNew(false)} onCreated={setNewResult} />}

      {/* Credenciais do aluno recém-criado — sempre mostra a password +
          WhatsApp quando o email real não pôde ser enviado */}
      <Dialog open={!!newResult} onOpenChange={o => !o && setNewResult(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Acesso de {newResult?.name} criado</DialogTitle>
          </DialogHeader>
          {newResult && (
          ...
          <DialogFooter>
            <Button type="button" variant="outline" className="min-h-[44px]" onClick={() => setNewResult(null)}>Fechar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
